...
export const requestPasswordResetOtp = functions.https.onCall(async (data, context) => {
  ...
  const expiresAt = admin.firestore.Timestamp.fromDate(
    new Date(Date.now() + RESET_OTP_EXP_MINUTES * 60 * 1000)
  );
...
export const resetPasswordWithOtp = functions.https.onCall(async (data, context) => {
  ...
  const ok = await bcrypt.compare(String(code), d.codeHash as string);
  if (!ok) {
    await ref.update({ attempts: attempts + 1 });
    throw new functions.https.HttpsError("invalid-argument", "Código incorrecto.");
  }
  ...
  const uid = (d.uid as string) || (await admin.auth().getUserByEmail(normEmail)).uid;
